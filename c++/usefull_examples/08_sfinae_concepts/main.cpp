...
// https://en.cppreference.com/w/cpp/feature_test
        ...
        template<typename T>
         // same as: std::enable_if_t<std::is_integral<T>::value, void> *
         // is just common to use: void * = nullptr 
         // because it's rarely missused
         T f(T t, std::enable_if_t<std::is_integral<T>::value>* = nullptr) {
            return t * t;
        }

        template<typename T>
       T f(T t, std::enable_if_t<std::is_floating_point<T>::value>* = nullptr) {
            return t * t * t;
        }
    #endif
